...
Image * img_ConvertGray(Image * origImg)
{
    int x, y;
    int j,i,soma;
    int soberba[3][3] = {{1,2,1},{0,0,0},{-1,-2,-1}};
    int soberba2[3][3] = {{1,2,1},{0,0,0},{-1,-2,-1}};
    ...
    for (y = 0; y < imgGetHeight(origImg); y++)
    {
        for (x = 0; x < imgGetWidth(origImg); x++)
        {
            for (j=-1; j<=1; j++)
            {
                for (i=-1; i<=1; i++)
                {
                    soma += soberba[j+1][i+1]*imgGetPixelGreen(origImg, x+i, y+j);
                }
            }
    ...
    for (y = 0; y < imgGetHeight(origImg); y++)
    {
        for (x = 0; x < imgGetWidth(origImg); x++)
        {
            for (j=-1; j<=1; j++)
            {
                for (i=-1; i<=1; i++)
                {
                    soma += soberba2[j+1][i+1]*imgGetPixelGreen(origImg, x+i, y+j);
                }
            }
